...
export function CoachBubble({ text, tone = 'neutral' }: { text: string | null; tone?: 'neutral' | 'good' | 'bad' }) {
  if (!text) return null;
  const ring = tone === 'good' ? 'border-accent' : tone === 'bad' ? 'border-danger' : 'border-edge-strong';
  return (
    <div className={`mt-3 flex gap-3 rounded-xl border ${ring} bg-surface-raised p-3`} role="note" aria-label={`${coachName} says`}>
