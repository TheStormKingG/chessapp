...
| ID | Requirement |
|---|---|
| F-RV-1 | Every game played in the app is analysed on the device by the engine at a fixed depth of about 14 (roughly half a second per position on the reference phone) with a visible progress bar, in under 60 seconds for a 40-move game. Key moments are then re-analysed at a higher depth. Imported games are analysed the same way. If analysis exceeds 90 seconds the review is shown with the moves analysed so far and completes in the background. |
