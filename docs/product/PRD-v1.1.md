...
| F-RV-3 | **Summary.** Accuracy for each side, the opening name with the move at which the game left the opening book, the move count by label, and the phase in which the game turned. |
| F-RV-4 | **Key moments.** Up to five moments chosen by the size of the swing in expected score, with at least one of each kind where present, a good move the learner found, a chance the learner missed, and the mistake that decided the game. At each moment the position is shown before the learner's move and the learner is asked to find the better move (retry before reveal). "Show me" reveals the answer. |
...
| F-RV-7 | **Fix it.** Where the game produced enough errors to build one, the review ends with a three-to-five-puzzle drill built from the error log, and a single suggested next lesson. A clean game ends without a drill rather than with one padded out of moves that were not errors. Completing the drill completes the plan's review item. |
...
| F-ER-2 | If a review exceeds its time budget, the partial review is shown, labelled as covering only the moves analysed, and banked for credit. Re-opening it re-analyses the whole game; nothing resumes in the background (F-RV-1). |
